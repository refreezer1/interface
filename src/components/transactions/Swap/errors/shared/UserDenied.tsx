...
export const hasUserDenied = (txError: SwapError) => {
  return (
    USER_DENIED_MESSAGES.some(message => txError.rawError.message.toLowerCase().includes(message.toLowerCase()))
  );
};

export const UserDenied: React.FC<ErrorProps> = ({ setState }) => {
  ...
  return (
    <Box sx={{ mt: 4, display: visible ? 'block' : 'none' }}>
      <Warning
        ...
        action={<CircularProgress variant="determinate" sx={{ opacity: 0.5, p: 0 }} value={progress} size={18} />}
      >
