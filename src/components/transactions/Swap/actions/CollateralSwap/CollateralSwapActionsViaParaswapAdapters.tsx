...
export const CollateralSwapActionsViaParaswapAdapters = ({
  ...
  const buildTxFn = async () => {
    ...
    const swapOut = {
      amount: normalize(state.swapRate?.destSpotAmount ?? '0', state.swapRate?.destDecimals ?? 18),
  ...
  const outputAmount = normalize(state.swapRate?.destSpotAmount, state.swapRate?.destDecimals);
