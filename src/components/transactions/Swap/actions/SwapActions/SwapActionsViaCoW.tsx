...
import { useSwapTokenApproal } from '../approval/useSwapTokenApproval';
...
export const SwapActionsViaCoW = ({
  ...
  const [user, estimateGasLimit, addTransaction] =
    useRootStore(
      useShallow((state) => [
        state.account,
  ...
  const {
    ...
    const {
      ...
      spender: isCowProtocolRates(state.swapRate) ? COW_PROTOCOL_VAULT_RELAYER_ADDRESS[state.chainId as SupportedChainId] : undefined,
      setState,
          ...
          try {
            if (await isSmartContractWallet(user, provider)) {
              const preSignTransaction = await getPreSignTransaction({
                ...
                chainId: state.chainId,
                user,
                amount: state.swapRate.srcSpotAmount,
