...
import { OrderType, SwappableToken, swappableTokenToTokenInfo, SwapParams, SwapState, SwapType, TokenType } from '../types';
...
export const SwapInputs = ({
  ...
  const handleInputChange = (value: string) => {
    ...
    if (state.orderType === OrderType.LIMIT && state.swapRate) {
      console.log('Manual edit should pause quote refresh', state.slippage, state.autoSlippage);
  ...
  const handleSelectedOutputToken = (token: SwappableToken) => {
    if (!state.destinationTokens?.find((t) => t.addressToSwap === token.addressToSwap)) {
      addNewToken(token).then(() => {
        setState({ destinationToken: token });
  ...
  useEffect(() => {
    ...
    fallbackOutputToken,
    state.chainId,
    state.sourceToken?.chainId,
    state.destinationToken?.chainId,
  ]);
