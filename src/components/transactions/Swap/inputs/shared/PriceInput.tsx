...
export const PriceInput = ({
  ...
  useEffect(() => {
    ...
    setAmount({
      fromAmount: normalizeBN(rateData?.srcSpotAmount, rateData?.srcDecimals),
      toAmount: normalizeBN(rateData?.destSpotAmount, rateData?.destDecimals),
      fromAmountUsd: normalizeBN(rateData?.srcSpotUSD, 0),
      toAmountUsd: normalizeBN(rateData?.destSpotUSD, 0),
    });
  }, [rateData]);
  ...
  const setNewRate = (newRate: BigNumberValue) => {
    ...
    if (rate.nominal && !rate.nominal.isZero() && rate.usd) {
      ...
      amount.toAmountUsd && amount.fromAmountUsd && amount.toAmount && amount.fromAmount &&
      !amount.toAmount.isZero() && !amount.fromAmount.isZero()
    ) {
