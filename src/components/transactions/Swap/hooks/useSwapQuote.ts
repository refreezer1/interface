...
import { SwapParams, SwapProvider, SwapQuoteType as SwapQuoteType, SwapState, TokenType } from '../types';
...
export const useSwapQuote = ({
  ...
  const quoteToState = (quote: SwapQuoteType | null | undefined) => {
    ...
    let slippage, autoSlippage, slippageValidation, safeSlippage;
  ...
  useEffect(() => {
    if (ratesError) {
      setState({ error: { rawError: ratesError, message: ratesError.message, actionBlocked: true }, ratesLoading: false, swapRate: undefined });
    }
  }, [ratesError]);
...
export const useMultiProviderSwapQuoteQuery = ({
  ...
  return useQuery<SwapQuoteType | null>({
    ...
    refetchInterval:
      !state.actionsLoading && !state.quoteRefreshPaused
        ? QUOTE_REFETCH_INTERVAL
        : false, // 30 seconds, but pause during action execution or when paused
  });
};
