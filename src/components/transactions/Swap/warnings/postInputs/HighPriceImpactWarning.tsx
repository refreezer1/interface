...
export function HighPriceImpactWarning({
  ...
  const lostValue = useMemo(() => {
    ...
    return valueLostPercentage(
      Number(state.swapRate?.destSpotUSD) * (1 - state.safeSlippage),
      Number(state.swapRate?.srcSpotUSD)
    );
  }, [state.swapRate, state.safeSlippage]);

  const showWarning = useMemo(() => {
    if (!state.swapRate) return false;
    return shouldShowWarning(lostValue, Number(state.swapRate?.srcSpotUSD));
